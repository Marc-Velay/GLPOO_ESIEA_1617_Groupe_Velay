...
public class EditorGardenUI extends JPanel implements ActionListener,MouseListener, MouseMotionListener{
	/**
	 * 
	 */
	...
	public static synchronized EditorGardenUI getInstance(int sizeX, int sizeY, int blocSize, GameObjects[][] gameMap, ArrayList<Kid> listKid, EditorGardenHUD hud) {
		if (instance == null) {
			instance = new EditorGardenUI(sizeX, sizeY, blocSize, gameMap, listKid, hud);
		}
		return instance;
	}
    ...
    private void drawPathKid(Graphics g, Kid kid) {
        ...
        if (!BLOCKED){
            if (xPathActuel<sizeX-1 && !gameMap[yPathActuel][xPathActuel + 1].isOccupied()){
                afficherImage(arrowRight,(xPathActuel+1)*blocSize, yPathActuel*blocSize,g);
            }
            if (xPathActuel>0 && !gameMap[yPathActuel][xPathActuel-1].isOccupied()){
                afficherImage(arrowLeft,(xPathActuel-1)*blocSize, yPathActuel*blocSize,g);
            }
            if (yPathActuel<sizeY-1 && !gameMap[yPathActuel+1][xPathActuel].isOccupied()){
                afficherImage(arrowDown,xPathActuel*blocSize, (yPathActuel+1)*blocSize,g);
            }
            if (yPathActuel>0 && !gameMap[yPathActuel-1][xPathActuel].isOccupied()){
		...
		if (!BLOCKED){
            ...
            if (etape==1) {
                ...
                if (e.getX() > blocSize && e.getX() < blocSize * (sizeX + 2) && e.getY() > blocSize+25 && e.getY() < blocSize * (sizeY)) {
                    ...
                    switch (objActuel) {
                        case Rock:
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setType(GameItemsList.Rock);
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setNumEggs(0);
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setOccupied(true);
                            break;
                        case Egg:
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setType(GameItemsList.Egg);
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setNumEggs(gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].getNumEggs() + 1);
                            break;
                        case Kid:
                            char oldDirection = kidActual.getDirection();
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setType(GameItemsList.Kid);
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setNumEggs(0);
                            ...
                            break;
                        case Empty:
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setType(GameItemsList.Empty);
                            gameMap[e.getY() / blocSize - 1][e.getX() / blocSize].setNumEggs(0);
                            break;
                ...
                if (e.getX() > xPathActuel*blocSize && e.getX() < (xPathActuel+1)*blocSize
                    ...
                    if (!gameMap[yPathActuel-1][xPathActuel].getType().equals(GameItemsList.Rock)){
                ...
                if (e.getX() > (xPathActuel+1)*blocSize && e.getX() < (xPathActuel+2)*blocSize
                    ...
                    if (!gameMap[yPathActuel][xPathActuel+1].getType().equals(GameItemsList.Rock)) {
                        switch (kidActual.getDirection()){
                            case 'E':
                                xPathActuel++;
	...
	private void saveMapAndKidMoves() {
        ...
        String nom = JOptionPane.showInputDialog(null, "Rentrez le nom de la carte", "Request", JOptionPane.QUESTION_MESSAGE);
        JOptionPane.showMessageDialog(null, "Carte enregistrée " + nom, "Message", JOptionPane.INFORMATION_MESSAGE);
            ...
            for (int y=1; y<sizeY-1; y++){
                for (int x=1; x<sizeX-1; x++){
                    if (gameMap[y][x].getType().equals(GameItemsList.Egg)){
                        pw.println("C " + x + "-" + y + " " + gameMap[y][x].getNumEggs());
                    } else if (gameMap[y][x].getType().equals(GameItemsList.Rock)){
                        pw.println("R " + x + "-" + y);
                    }
                }
            }
